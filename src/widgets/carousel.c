...
/* Slide distance that guarantees both icons are off-screen at animation
 * start/end, regardless of each icon's individual width. */
...
void GfxCarouselNext(GfxCarousel *c)
{
    ...
    int out_w = GfxIconWidth(&c->items[old].icon,         ah);
    c->_anim_offset = slide(c, in_w, out_w);
}
...
void GfxCarouselDraw(GfxRenderingTile *tile, GfxCarousel *c)
{
    ...
    /* Incoming (selected) icon — centred horizontally + animated. */
    int in_w  = GfxIconWidth(&c->items[c->selected].icon, ah);
